...
Web API テストをユニットスコープで行うというのは、Vitest などをつかってフレームワークのテスト機能を活用して関数の単位などでテストを行うこと。
DB さえ用意してしまえば、データの差し込みや改変を関数内部で自由に行えるため、テストがしやすい一方、究極的な意味での横断的網羅的なテストではない。

Web API テストを E2E スコープで行うというのは、全ての関連するインフラを立ち上げて、API リクエストによりテストを行うこと。
