...
コンポーネントが追加または更新されると、更新のコンテキスト（by ユーザー操作、by データ取得、by バックグラウンド処理）や、
コンポーネントの可視性、`useTransition`で呼ばれたか、などの要素を考慮し、適切なレーンが選択される。

Fiber Concilerは、優先度の高いレーンから処理していく。
同じレーンの処理はBatchingされる。
1つのレーンを処理するごとにCommit Phaseを経る(DOMの更新とEffectsの実行)。
全てのレーンが処理されるまで、処理を続ける。
...
Suspenseと組み合わせて使うと便利。
なぜかというと、画面遷移やタブ切り替えの際に、優先度の低いバックグラウンドでデータ読み込みと描写をしつつ、
準備中は現在のUI+ローダーを表示しておき、準備ができた段階で切り替える、といったことが可能になるからだ。
...
Stateのうち、更新が重たい描写を引き起こし、かつ優先度は低くても構わない、というものに使うと良い。
stale-while-revalidateの考えに基づいている。

主なユースケースは以下である。
ただし、逆にラギーなUXにならないよう、ユーザー入力に起因するものにだけ使うこと。
