...
**useMemoは、コンポーネント内において、高価な計算を避けたり、結果への参照を一貫させる**ためのもの。

使用例としては、親から与えられた配列をコンポーネント内でソートする場合など、non-primitiveな値に対する計算処理に使う。
プリミティブな値に対してつかうメリットはない。メモ化しようが、どのみち同じ計算が走り、意味がないため。
