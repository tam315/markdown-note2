...
- `renderToString`(Node.js環境用)
  - JSXを受け取り、HTML stringを返す
  - 同期なので:
    - パフォーマンスに問題がある(リクエストが増えるとすぐにイベントループを占拠するようになる)
    - TTFBが遅い
- `renderToPipableStream`(Node.js環境用)
  - JSXを受け取り、Node.jsのReadableStreamを返す
  - 非同期
  - ネットワークを超えてストリームが可能(Suspense対応)
  - 全ページのレンダリングを待たずに、できた部分から送信し始めるので、TTFBがはやい
- `renderToReadableStream`(ブラウザ環境用)
  - 前項の関数をNode.js環境ではなくブラウザ環境で動作させたい場合に使う
