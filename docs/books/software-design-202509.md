...
- トランクベース + gRPCのProtocol BufferによるFeature Flag で開発
- サーバサイドのE2Eテストは環境構築の複雑性と実行時間を考慮して、開発環境でのみ実行
- Linearという、エンジニアチームに特化したプロジェクト管理ツールを採用
- gRPCをwebから使うときはプロキシが必要になるが、gRPC/Connectの採用でプロキシを不要にしてシンプル化(週間ダウンロードはまだ30万程度ではある)
- モバイルアプリのテストをGitHub Actions内で完結させることは可能 but how
- 重要な依存ライブラリは月イチで手動更新、周辺ツールなどの依存ライブラリでマイナー・パッチレベルのものは週イチで自動マージ
- MagicPodというE2Eテストを自動化するクラウドサービスを利用
- TanStack+gRPC=Connect-Queryを使っている。要件により、楽観更新で済ませるところと実際にリクエストするところがある。
