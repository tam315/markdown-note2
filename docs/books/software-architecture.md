...
大枠として同期通信か非同期通信かをまず決める必要がある。

同期通信の場合、「プロトコルを意識した異種間相互運用性」を利用する。

- 「プロトコルを意識」他のサービスを呼び出すには使い方を知る必要があるから
- 「異種間」サービスが違えばプラットフォームも違うから
- 「相互運用性」サービスが互いに呼び合うから

非同期通信の場合、イベントやメッセージを使い、内部的にはイベント駆動アーキテクチャを利用する。
この際、ブローカーパターンやメディエーターパターンは、マイクロサービスではコレオグラフィとオーケストレーションとして使われる。
