...
リクエストの80%がいわゆるただのバケツリレーになっているなら、このアーキテクチャは適切でないことを示しているかも。
...
コアシステムとプラグインの通信は、通常はPoint to Point(同期)である。
RESTのように非同期にすることも可能だが、分散システムになるのでデメリットもある。

プラグインが大量にある場合は、発見&取得できるようにレジストリを作ることもある。

コントラクト（コアシステムとプラグインの間のデータ受け渡しに関する規約）は、全体で統一した標準を定める。
...
- パフォーマンスはやや高い - 余計なものを排除でき、レイヤードのように肥大化しないので
