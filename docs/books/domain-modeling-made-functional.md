...
最初の打ち合わせでは一つのワークフロー（例えば商品の注文に関する部分）を高レベルな視点で把握する。
...
ANDしたものを`product types`と呼ぶ。
ORしたものを`choice types`/`tagged unions`/`sum types`/`discriminated unions`などと呼ぶ。
