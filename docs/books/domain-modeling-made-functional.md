...
なので、多くの場合は **結果整合性** を確保することで行われる。

コンテキスト間の調整を避けるため、単にメッセージを送るだけにしよう。
メッセージが消失した場合の対応には3つの選択肢がある。

- 何もしない
  - 影響が軽微だったり、やり直せばいいだけの場合
- 調整を行う
  - 各コンテキストのデータを突合して、一致しなければメッセージの再送を促すプロセスを作る
- 補償アクション(compensation action)を実行する
  - 前のアクションを「もとに戻す」か、エラーを修正する
