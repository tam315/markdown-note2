...
多段if文を使って何らかの計算をするようなコードは、拡張するたびに意図が不明瞭になっていく。
こういうときは**Active Pattern**を使って型の力を使うことで、
処理漏れがなくなり、意図も明確に伝わるようになる。
...
安定して稼働している既存のステップや出力の型を拡張するのではなく、
新しいステップと新しい型を作るほうがよい。
そうすることでステップ間の独立性が高まり、事故が減り、削除も簡単になる。

### ビジネスルールの出力ではなく入力をモデル化する

例えばVIPユーザーは送料無料になるというビジネスルールを追加する場合。

ビジネスルールの出力(`送料無料`)をモデル化してはダメ。
ビジネスルールへの入力(`VIPユーザー`)を保持し、入力に対して計算をするようにモデル化しろ。
