...
- 独自のビルドチェーンなら、まあ頑張れ
  - `tsc`で全て JS として出力し、その結果に対して既存の処理を適用するのが王道だろう

## 82. 依存グラフの端っこから取り掛かる

JS を TS に変換するときは、依存グラフの端っこから取り掛かると進めやすい。
つまり、import が少ないコードから手を付けていく。
[madge](https://github.com/pahen/madge)などのツールを使うと依存グラフを可視化できる。

- まずはサードパーティーライブラリから手を付ける
  - だってサードパーティーが我々のコードをインポートすることはないから
  - `@types/*`をインストールする
- 次に手を付けるのは util 系のコードになるだろう
- リファクタの衝動を抑えて、TODO コメントを書きつつ一旦 TS への変換をやり切る
- JSDoc によるチェックは効かなくなるので、Quick Fix で TS の型に変換するのを忘れずに

TS への変換時にありがちな問題は以下の通り

- 未宣言のクラス属性
  - Quick Fix + 手動で頑張る
- 型が途中で変わるエラー
  - ひとまず型アサーションで対処し、後でリファクタする

## 83. `noImplicitAny`は必ず有効にする

`strict`とか`strictNullChecks`とか色々オプションはあるけど、最も重要なのが`noImplicitAny`である。
これを有効化しない限りは他のオプションは意味をなさないし、TypeScript に変換が完了したとも言えない。
