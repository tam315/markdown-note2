...
Open-host service(OHS)を使ってupstream側で行われる場合がある。

### ステートレスなモデル変換

ステートレスな変換のうち、**同期的**なものは、リクエストが発生する都度にOHSかACLにおいて行われる。
いずれの場合も、**Proxy design pattern** を使うことになる。
...
システムの規模などによっては、APIゲートウェイパターンなどを使って
