...
データベース、外部サービス、UI、メッセージバスムなどである。

そして、従来のように Business layer が直接 Infra layer を呼び出すのではなく、
Business layer は **Port（規格・インターフェース）**を定義し、Infra layer は **Adapter（実装）**を定義する。
これにより、依存の方向が Infra -> Business logic という従来とは逆方向になり、
...
CQRSは、ビジネスロジックと技術を分離するという考え方は Port & Adapters と共有しているが、
複数のモデルを使う **Polyglot modeling** である点が特徴である。

なぜ複数のモデルが必要になるか？
例えば業務で使うモデルと、分析に使うモデルは、自ずと異なってくるだろう。
また、目的によって複数の種類のDBを使い分けたい場合にも、複数のモデルが必要になってくるだろう。

もともとCQRSは Event-sourced model のイベントをクエリする際に、
1つの集約に関するイベントしか取れなくて不便であるという問題を解決するために生まれた。
CQRSを使えば、イベントを投影済みの集約を、物理DBに格納して柔軟にクエリが可能になるのだ。

**Command execution model**は更新用モデル。
ビジネスロジック、バリデーションルール、不変条件の強制、などを担当する。
強い一貫性を必要とする唯一のモデルであり、Source of Truth である。
強い一貫性が保証された形でデータを読み込める必要がある。
更新時には同時性を制御するための楽観的ロックの仕組みが必要である。

**Read model**はクエリ用モデル。
DB上、ファイル上、メモリ上など、どこにでも住める。
あらかじめキャッシュされた、イベントを投影済みのモデルである(RDBのmaterialized viewに近い)。
モデルはいつでも一から再生成できるので、投影済みのモデルを削除したり、
将来的に別のモデルを追加することも容易である。
モデルは用途に応じていくつ作ってもいい。
Read-only モデルであり、モデルの一部を書き換えたりすることはできない。

Projectionの動作には同期と非同期がある。
**Synchronous Projection**の場合は、以下の流れになる。

- Projection engine は Online Transaction Processing (OLTP) 用のDBに対し、(指定したチェックポイント以降に)追加 or 更新されたレコードをクエリする
- Projection engine は、そのデータをもとに Read model を再生成 or 更新する
- Projection engine は、処理したレコードについてチェックポイントを内部的に保持しておき、次回以降の処理はそれ以降の部分に関してのみ行う

前提として、OTLPのレコードはチェックポイント番号を保持し、
チェックポイント番号をもとに増分レコードだけを抽出できるようになっている必要がある。
投影モデルをゼロから再生成したいときは、単にチェックポイントを`0`などにすればすむ。

**Asynchronous Projection**では、Command execution model が全ての変更をメッセージバスに発行する。
Projection engine は、メッセージバスからイベントを受信し、リードモデルを更新する。
この方法はスケールアウトするのは間違いないが、一方で、
処理順が変わったり重複実行したときには一貫性のないデータができる恐れがある。
また、新しい投影モデルを追加したり、既存の投影モデルをゼロから再生成するのは難しい。
