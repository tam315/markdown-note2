...
より洗練された方法の基礎となる書き方である。

一番のメリットはそのシンプルさである。理解しやすくパフォーマンスも良い。
これは諸刃の剣で、ロジックが複雑になるとすぐにコードが重複して管理が難しくなり、一貫性のない挙動を生む。
ETLのような、ロジックがシンプルなSupporting Domainに最適。
Core Domainに使われるべきではない。

### Active Record

ロジックはシンプルだがデータ構造が複雑なものに最適。
Active Recordと呼ばれるオブジェクトを使う。
このオブジェクトは、データ構造と、データにアクセスするためのCRUDメソッドを持つ。
