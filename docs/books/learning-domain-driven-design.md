...
特定のアクターが、一定の範囲のデータとそれに対応するユースケースを使って、
業務フローを完結できる単位が、一つのサブドメインになる。
...
サブドメインは**発見**するもの。企業の戦略で決まり、開発者の範疇の外にある。
関連するユースケース群から構成される。

Bounded Context(=ユビキタス言語のスコープ)は**デザイン**するもの。
戦略的な判断により適切に決める。必ずしもサブドメインと1-1で結びつかなくてもいい。
基本的には問題領域の数や大きさに応じて分ける。
小さいほうがモデルの一貫性を保ちやすく、管理しやしく、スケールアウトしやすい。
...
コードが重複するコストが、もろもろの調整コストを上回る場合に使用を検討する。
変更を加えたら自動でCIが実行され、即時に全てのContextに反映される。
影響を小さくするため、モデルは必要最小限の範囲とすべき。
代表的なユースケースとしては以下がある。
