...
IoCとは逆に、依存に直接依存したり、依存を直接制御したりすること。
合成基点の外で、揮発性依存の実装を`new`することで発生する。
Constructor Injectionなどへの適切なリファクタリングが必要。
...
合成基点の外で、揮発性依存の取得を無制限に行えるようにするアンチパターン。

依存を必要とするクラスに、その依存を**外部から見えない形で**提供できるようにする。
ただし、目的の依存が必ず提供されることは保証されていない。

DIコンテナもサービスロケーターもほぼ同じだが、合成基点の外で使っちゃダメだよね、という視点が大事。

### Ambient Context

staticなアクセス・メソッドを介して、1つだけしか存在しない依存のオブジェクトを取得できるようにすること。

### Constrained Construction

コンストラクタに特定のシグネチャを持つことを強いること。
